...
export default function Dashboards() {
  ...
  const datasets = allDatasets.filter(
    (d) => d.columns?.length > 0 && d.row_count > 0,
  );
...
KPI cards should have label, value (as string with formatting), trend (up/down/stable), and change (percentage string).`,
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      <PageHeader
        title="Paneles"
        subtitle="Paneles generados por HELIOS a partir de tu intención analítica"
      />
          ...
          <div className="relative">
            ...
            {datasets.length === 0 && (
              <p className="text-xs text-amber-400 mb-3 flex items-center gap-1">
                ⚠ No hay conjuntos de datos con información procesada. Carga y
      ...
      {dashboards.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-xs text-slate-600">
            Describe lo que quieres analizar y HELIOS generará un panel
            automáticamente
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-3">
                  <span className="text-[10px] text-slate-600">
                    {db.kpi_cards?.length || 0} KPIs · {db.charts?.length || 0}{" "}
                    charts
                  </span>
                </div>
              </button>
